...
class BetsController extends Controller
{
    public function store(Request $request): RedirectResponse | View
    {
        ...
        $upcomingMatches = DB::table('events')->where('status', 'like', 'upcoming')->orderBy('id')->get();
        ...
        foreach ($upcomingMatches as $um) {
            $um=json_decode(json_encode($um), true);
            $oddse = DB::table('odds')->where('event_id', 'like', $um['id'])->get();
            $odds = json_decode(json_encode($oddse[$i]), true);
            if ($bet_code == '0') {
                continue;
            }
            ...
            DB::table('odds')->where('event_id', '=', $um['id'])->update([
                'sum' => $odds['sum'] + $request["amount"],
            'win_op_1' => $odds['win_op_1'] - 0.1,
            'win_op_2' => $odds['win_op_2'] - 0.1]);
        ...
        foreach ($upcomingMatches as $um) {
            ...
            $bet = json_decode(json_encode($bets[$i]), true);
            $um = json_decode(json_encode($um), true);
